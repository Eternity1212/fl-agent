...
> ⚠️ **公平性关键(median/trimmed)**: 坐标中位数/截尾均值理论容错为 **< 50% 拜占庭客户端**(n≥2f+1)。
> 我们主设定 het04 = 客户端2,3 脏 = **50% 脏(2/4), 恰在/超过击穿点** → median 崩到接近随机(0.4739)
> 是**"用超规格"的预期行为, 不能写成"鲁棒聚合无效"**(会被审稿人一击致命)。K=8 脏4/8 仍是 50%, 同理会崩。
> **修复**: scale 矩阵新增 `het04lo_*`(25% 脏, 仅客户端3脏)规格内对照 → 若 25% 正常、50% 崩,
> 得到"击穿点跨越"叙事(攻不破), 反衬高脏比下自适应重加权的价值。

> 📌 **指标纪律再强调**: 排名一律用 **macro-AUROC**, 不用 best-*-F1(RFMiD het02 下全方法 F1 都低,
> FedAvg best-micro-F1 也只有 0.30, 是 regime 通病)。seed0 实测 het02: FedA3I AUROC 0.7443 **高于**
> FedAvg 0.7211 / Robust 0.7302, 仅逊于 agent 0.7812 → FedA3I 是"中上/名义强 baseline", **不是"偏弱"**。
