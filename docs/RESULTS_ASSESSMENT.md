...
| Agent | 0.5330 | 0.0640 | 0.000 | **塌陷(输给 FedAvg −0.055)** |
| agentmu | 0.6040 | 0.0746 | 0.000 | 部分回收但仍弱 |
...
### 表 5 — ⚠️ RHFL/CCR 首批结果(2026-07-01):强基线,当前正面赢过 agent

同 seed 对比(macro-AUROC):

| 条件 | seed | FedAvg | Agent(ours) | **CCR** | CCR−Agent |
|---|---|---|---|---|---|
| het02 | 0 | 0.7211 | 0.7812 | 0.7765 | −0.005(平) |
| het04 | 0 | 0.7374 | 0.7908 | **0.8149** | **+0.024(CCR赢)** |
| het04 | 1 | 0.7377 | 0.8102 | **0.8284** | **+0.018(CCR赢)** |

> **关键警报**:在**主打条件 het04 上,CCR 两个 seed 都赢过我们的门控**(+0.02 左右),het02 打平。
> 原因:我们的 sigmoid 门控以中位数为中心、偏**保守**(干净客户端保持均衡、只压明显离群);CCR 的
> softmax(温度0.05)极度**激进**,几乎把权重全给分数最高的那个干净客户端 ≈ "只用最好的客户端"。
> 在 het04(4个里2个脏)下,激进集中恰好有利 → CCR 更高。
>
> **含义**:"我们的自适应加权在噪声下更准"这个卖点**受到实质威胁**。真正能站住的差异化,转向
> CCR 的**软肋**:(a) **clean 零代价**——CCR 即使全干净也只用1个客户端、丢掉3/4数据,预期掉点;
> (b) **非IID**——CCR 过度集中预期塌陷。这两点是新增 `clean_ccr` / `dir_ccr` 对照要验证的。
> ⚠️ 仅 2 seed, 待 s2 补齐;但两个 matched seed 一致 CCR 赢,不像随机噪声。
